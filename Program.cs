...
CLR (C+) -> native
assembler (язык железа, высоко уровненный)
Главная особенность CLR она позволяет преобразовывать на язык С# в промежуточный язык IL (C# -> IL)
Таким образом мы можем собирать приложения из разных частей других языков, например кусок на VB.Net мы также может преобразовать в IL и запустить приложение  (VB.Net -> IL)
Это некоя прослойка.

|ВЫСШЫЙ УРОВЕНЬ| -> |IL| -> |native|

Для то чтобы языки смогли работать вместе, существует соглашение CLI - есть определённые редакции языков программирования, которые следуют данному соглашению, что даёт 
нам сочетать в приложении разные языки программирования.
В некоторых случаях существует возможность делать различные сборки.

Основные понятия:
CLI - command language infrastructure (соглашение)
CTS - command type system (одни типы)
CLS - command language system (языковые конструкции(чтобы включал в себя например for, foreach))
Соглашение требует использовать похожие методы и типы.
CLR - конкретная набор служб, набор библиотек, в которой выполняется на приложение следуя соглашению CLI

Внутри CLR находиться две крупные библиотеки:
BCL - базовые классы
